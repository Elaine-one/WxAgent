...
TOOL_META = ToolMeta(
    ...
    description="媒体工具集：音频转录、视频字幕、OCR识别",
    version="1.0.0",
    tags=["media", "audio", "video", "ocr"],
)
    ...
    try:
        result = subprocess.run(
            ["ffmpeg", "-y", "-i", silk_path, "-acodec", "pcm_s16le",
             "-ar", str(config.ADV_FFMPEG_SAMPLE_RATE), "-ac", "1", wav_path],
            capture_output=True, timeout=config.ADV_SILK_DECODE_TIMEOUT,
        )
        if Path(wav_path).is_file():
            return wav_path
...
def _transcribe_audio(file_path: str, language: str = "zh",
    ...
    try:
        ...
        model = WhisperModel(config.ADV_WHISPER_MODEL, device=config.ADV_WHISPER_DEVICE, compute_type=config.ADV_WHISPER_COMPUTE_TYPE)
...
def _transcribe_via_api(file_path: str, language: str) -> ToolResult:
    ...
    try:
        llm = create_llm(LLM_PROVIDER, LLM_API_KEY, LLM_BASE_URL, config.ADV_WHISPER_CLOUD_MODEL)
...
def _do_subtitle_task(video_path: str, target_language: str = "zh") -> str:
    ...
    subprocess.run(
        ...
        capture_output=True, timeout=config.ADV_FFMPEG_AUDIO_EXTRACT_TIMEOUT,
    )
...
def _ocr_image(image_path: str, output_format: str = "text",
    ...
    try:
        ...
        ocr = PaddleOCR(use_angle_cls=True, lang=config.ADV_OCR_LANG)
...
def _ocr_via_vision_api(image_path: str, output_format: str) -> ToolResult:
    ...
    from config import LLM_PROVIDER, LLM_API_KEY, LLM_BASE_URL
    try:
        llm = create_llm(LLM_PROVIDER, LLM_API_KEY, LLM_BASE_URL, config.ADV_OCR_FALLBACK_MODEL)
