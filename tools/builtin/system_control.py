...
TOOL_META = ToolMeta(
    ...
    description="系统控制工具集：进程管理、端口检查、应用启动",
    version="1.0.0",
...
def _open_app(app_name: str, state=None, user_id: str = "") -> ToolResult:
    ...
    if not cmd:
        ...
        return ToolResult(
            success=False,
            error=f"❌ 应用「{app_name}」不在白名单中，无法打开。\n\n✅ 白名单中的应用：{available_str}\n\n💡 提示：请在 config.yaml 的 app_whitelist 中添加此应用。",
            display=f"应用「{app_name}」不在白名单中",
        )
    try:
        subprocess.run(cmd, shell=True, capture_output=True, text=True, timeout=10)
        return ToolResult(success=True, content=f"✅ 已启动应用：{app_name}", display=f"已启动：{app_name}")
    except Exception as e:
        return ToolResult(success=False, error=f"启动失败：{e}", display=f"启动失败：{app_name}")
