...
WxAgent 智能体的 Web 管理面板，提供可视化配置、服务管理、日志查看、工具管理等功能。
...
```
src/
├── App.tsx                  # 路由定义（13 个页面路由）
├── main.tsx                 # 应用入口
├── index.css                # 全局样式
│
├── api/
│   └── client.ts            # Axios API 客户端（21 个 API 函数）
│
├── assets/                  # 静态资源
│   └── hero.png
│
...
│   ├── Security.tsx         # 安全设置：风险分级、路径沙箱、AI 审查器
...
│   ├── Scenarios.tsx        # Skill 管理：查看/AI 生成/创建/删除 Skill（即场景模式）
│   ├── SystemControl.tsx    # 系统控制：系统操作定义、应用白名单
│   ├── McpManage.tsx        # MCP 管理：MCP 服务器连接/断开/工具浏览/启停
│   └── FeishuManage.tsx     # 飞书管理：飞书连接测试/文档浏览/多维表格管理
│
└── store/
...
| `/scenarios` | Scenarios | Skill 管理与生成（即场景模式） |
| `/system` | SystemControl | 系统控制配置 |
| `/mcp` | McpManage | MCP 服务器管理 |
| `/feishu` | FeishuManage | 飞书连接与文档管理 |
...
| `getMcpStatus` | GET | `/mcp/status` | 获取 MCP 服务状态 |
| `getMcpTools` | GET | `/mcp/tools` | 获取 MCP 工具列表 |
| `connectMcpServer` | POST | `/mcp/connect/{name}` | 连接 MCP 服务器 |
| `disconnectMcpServer` | POST | `/mcp/disconnect/{name}` | 断开 MCP 服务器 |
| `toggleMcp` | POST | `/mcp/toggle` | 启停 MCP 服务 |
| `getFeishuStatus` | GET | `/feishu/status` | 获取飞书连接状态 |
| `testFeishuConnection` | POST | `/feishu/test-connection` | 测试飞书连接 |
| `getFeishuDocuments` | GET | `/feishu/documents` | 获取飞书文档列表 |
| `getFeishuBitables` | GET | `/feishu/bitables` | 获取飞书多维表格 |
| `deleteFeishuDocument` | DELETE | `/feishu/documents/{fileToken}` | 删除飞书文档 |
...
- `status` — 服务运行状态（running/pid/uptime/ready）
- `startError` — 启动失败时的错误详情
- `fetchStatus()` — 查询服务状态
